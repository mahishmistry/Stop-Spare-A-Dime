...
function _assert_valid_date(value, field_name) {
 ...
 */
export async function add_user(email, name) {
    _assert_valid_email(email);
    const result = await pool.query(`INSERT INTO users (email,name)
        VALUES ($1, $2)
        RETURNING *;`, [email, name ?? null]);
 ...
 */
export async function add_store(source, url) {
    _assert_non_empty_string(source, "source");
    _assert_non_empty_string(url, "url");
    _assert_valid_url(url);
    const result = await pool.query(`INSERT INTO stores(store_source,url)
         VALUES ($1, $2)
 ...
 */
export async function get_store_by_source(source) {
    _assert_non_empty_string(source, "source");
    const result = await pool.query(`SELECT * 
        FROM stores
        WHERE store_source = $1;`, [source]);
    return result.rows[0] ?? null;
}
;
/**
 ...
 */
export async function add_store_location(source, address) {
    _assert_non_empty_string(source, "source");
    ...
    const result = await pool.query(`INSERT INTO store_addresses(store_source, address)
        VALUES ($1, $2)
        RETURNING *;`, [source, full_address]);
    return result.rows[0];
}
;
/**
 ...
 */
export async function _add_brand(name) {
    _assert_non_empty_string(name, "name");
    const result = await pool.query(`INSERT INTO brands (name)
        VALUES ($1)
        RETURNING *;`, [name]);
    return result.rows[0];
}
 ...
 */
export async function _add_category(name) {
    _assert_non_empty_string(name, "name");
    const result = await pool.query(`INSERT INTO categories (name)
        VALUES($1)
        RETURNING *;`, [name]);
    return result.rows[0];
}
 ...
 */
export async function _add_unit(name, unit_type) {
    _assert_non_empty_string(name, "name");
    _assert_non_empty_string(unit_type, "unit_type");
    const result = await pool.query(`INSERT INTO units (name, unit_type)
        VALUES ($1, $2)
        RETURNING *;`, [name, unit_type]);
    return result.rows[0];
}
;
/**
 ...
 */
export async function add_product(name, brand_id, category_id, unit_id, ebt_eligible) {
    _assert_non_empty_string(name, "name");
    if (brand_id != null) {
    ...
    const result = await pool.query(`INSERT INTO products (name, brand_id, category_id, unit_id, ebt_eligible)
        VALUES ($1, $2, $3, $4, $5)
        RETURNING *;`, [
        name,
        brand_id ?? null,
 ...
 */
export async function get_product_by_id(product_id) {
 ...
 */
export async function get_product_by_name(name) {
    _assert_non_empty_string(name, "name");
    const result = await pool.query(`SELECT *
        FROM products
        WHERE name = $1;`, [name]);
 ...
 */
export async function add_item(product_id, store_source, store_item_id, avg_rating, rating_count, package_quantity, unit_id) {
    _assert_positive_integer(product_id, "product_id");
    ...
    const result = await pool.query(`INSERT INTO items
 ...
 */
export async function get_item_by_store_item_id(store_source, store_item_id) {
    _assert_non_empty_string(store_source, "store_source");
    _assert_positive_integer(store_item_id, "store_item_id");
    const result = await pool.query(`SELECT *
        FROM items
        WHERE store_source = $1 AND store_item_id = $2;`, [store_source, store_item_id]);
    return result.rows[0] ?? null;
}
;
/**
 ...
 * @returns A Promise that resolves to an item object with item_id, product_id, store_item_id, avg_rating, rating_count, package_quantity, unit_id, and store_source, or null.
 */
export async function get_item_by_id(item_id) {
 ...
 */
export async function get_item_by_name(name) {
    _assert_non_empty_string(name, "name");
    const result = await pool.query(`SELECT items.*
        FROM items
        JOIN products ON items.product_id = products.product_id
        WHERE products.name = $1;`, [name]);
    return result.rows;
}
;
/**
 ...
 */
export async function add_deal(item_id, original_price, on_sale, last_fetched, sale_price, membership_sale) {
    _assert_positive_integer(item_id, "item_id");
 ...
 */
export async function get_deal_by_id(deal_id) {
    _assert_positive_integer(deal_id, "deal_id");
    const result = await pool.query(`SELECT *
        FROM deals
        WHERE deal_id = $1;`, [deal_id]);
    return result.rows[0] ?? null;
}
;
