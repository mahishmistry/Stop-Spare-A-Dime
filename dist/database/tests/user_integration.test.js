...
import { initialize_pool, pool } from "../../dist/database/pool.js";
beforeAll(async () => {
...
function _unique_suffix() {
    return `${Date.now()}_${Math.floor(Math.random() * 1000000)}`;
}
async function _insert_test_product(product_name_prefix = "Product") {
    const product_name = `${product_name_prefix}_${_unique_suffix()}`;
    const req = await pool.query("INSERT INTO products (name) VALUES ($1) RETURNING product_id", [product_name]);
    return {
...
async function _insert_test_deal() {
    ...
    const item_req = await pool.query("INSERT INTO items (product_id, store_item_id, store_source) VALUES ($1, $2, $3) RETURNING item_id", [product_id, Math.floor(Math.random() * 1000000000), store_source]);
    const item_id = item_req.rows[0].item_id;
    const deal_req = await pool.query("INSERT INTO deals (item_id, original_price, on_sale, sale_price) VALUES ($1, $2, $3, $4) RETURNING deal_id", [item_id, 9.99, true, 7.49]);
    return {
...
describe("Memberships", () => {
    ...
    it("Handles favoriting of non-existent memberships", async () => {
        const user_context = await _test_user_context();
        await expect(user_context.add_store_membership("non_existent_store", "1234-1525-12591RSD")).rejects.toThrow();
    });
...
describe("Search History", () => {
    ...
    it("Allows the user to delete a search query", async () => {
        ...
        await user_context.add_search_history("milk", new Date("2026-04-01T10:00:00.000Z"));
        await pool.query("DELETE FROM search_history WHERE user_id = $1 AND search_query = $2", [user_id, "milk"]);
        const search_history = await user_context.get_search_history();
    ...
    it("Handles deletion of search queries that aren't saved", async () => {
        ...
        const delete_req = await pool.query("DELETE FROM search_history WHERE user_id = $1 AND search_query = $2", [user_id, "query_that_does_not_exist"]);
        expect(delete_req.rowCount).toBe(0);
    });
    it("Handles saving of non-existent search queries", async () => {
        const user_context = await _test_user_context();
        await expect(user_context.add_search_history(null, new Date("2026-04-01T10:00:00.000Z"))).rejects.toThrow();
    });
    it("Handles deletion of non-existent search queries", async () => {
        const { user_context, user_id } = await _test_user_context_with_id();
        await pool.query("DELETE FROM search_history WHERE user_id = $1 AND search_query = $2", [user_id, "another_missing_query"]);
        const search_history = await user_context.get_search_history();
        expect(search_history.length).toBe(0);
    });
    it("allows two different users to have independent search history", async () => {
        ...
        await pool.query("DELETE FROM search_history WHERE user_id = $1 AND search_query = $2", [2, "query_a"]);
        // User 2 adds query_b
...
//# sourceMappingURL=user_integration.test.js.map