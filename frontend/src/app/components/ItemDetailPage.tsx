...
import { addFavorite, removeFavorite, addBookmark, removeBookmark } from "../services/products.ts";
...
export function ItemDetailPage({
  ...
  const handleToggleBookmark = async () => {
    ...
    try {
      ...
      // Using product name as deal identifier for now
      const dealId = item.name.toLowerCase().replace(/\s+/g, "-");
      if (newState) {
        await addBookmark(parseInt(dealId.length.toString()));
      } else {
        await removeBookmark(parseInt(dealId.length.toString()));
      }
